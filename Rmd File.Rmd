...
vert <- sample(1:vcount(igraph_ob), 20)
vert2 <- sample(1:vcount(igraph_ob), 20)

set.seed(3)

x <- sample(1:vcount(igraph_ob), 20)

gen_plot <- induced.subgraph(igraph_ob, x) 
                             
                             V(igraph_ob)[vert2])

plot(gen_plot, vertex.color = ifelse(V(igraph_ob)$Gender[x] == "F", "purple", "yellow"), vertex.label = V(igraph_ob)$Gender[x])
