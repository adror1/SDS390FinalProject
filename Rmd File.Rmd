...
ergm_net <- network(ergm_mat, vertex.attr = ergm_node_att,
                    vertex.attrnames = colnames(ergm_node_att), directed = F)

igraph_ob <- asIgraph(ergm_net)
V(igraph_ob)$color <- ifelse(igraph_ob$Gender == "F", "pink", "blue") 

vertex.attributes(igraph_ob) <- list(gender = node_characteristics_final$Gender)

plot(igraph_ob vertex.color = gender)

plot(igraph_ob) %>%
  set_vertex_attr("gender", value = ergm_net$Gender)

# ergm_node_att <- node_characteristics_final %>%
#   mutate(gen_num = ifelse(Gender == "M", 0, 1)) %>%
#   mutate(race_num = ifelse(Race == "White", 0, 1)) %>%
#   mutate(party_num = ifelse(Party == "Republican", 0, 1))
# library(intergraph)
# 
# 
# 
...
# z <- sample.int(105, 20)
# 
# set.vertex.attribute(ergm_net, "gender", as.character(ergm_net$Gender))
# 
# gen.color<-rep("yellow", 105)
# gen.color[ergm_net$Gender=="F"]<-"blue"
# 
# set.vertex.attribute(ergm_net, "gender.col", (gen.color))
# 
# plot(test, vertex.color = V(test)$gen.color)
# 
# test <- asIgraph(ergm_net)
# test2 <- subgraph(test, y)
# plot(test2) 
# 
